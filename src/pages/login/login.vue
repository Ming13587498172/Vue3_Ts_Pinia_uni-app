...
const onGetphonenumber: UniHelper.ButtonOnGetphonenumber = async (e) => {
  try {
    ...
    // 登录请求
    const result = await postLoginWxMinAPI({ code, encryptedData, iv })
    // loginSuccess(result)
  } catch (err) {
...
const onGetphonenumberSimple = async () => {
  try {
    const { result } = await postLoginWxMinSimpleAPI('13112233445')
    loginSuccess(result)
  } catch (err) {
...
// 登录成功
const loginSuccess = async (result: LoginResult) => {
  // 保存用户信息
  useMemberStore().setProfile(result)
  // 成功提示
