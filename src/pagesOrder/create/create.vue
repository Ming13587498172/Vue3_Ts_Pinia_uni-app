...
// 收货地址
const selectAddressData = computed(() => {
  return selectedAddress.value || orderPre.value?.userAddresses.find(item => item.isDefault)
})
...
const getMemberOrderPreData = async () => {
  try {
    if (query.skuId && query.count) {
      const { result } = await getMemberOrderPreNowAPI({ skuId: query.skuId, count: query.count })
...
const onOrderSubmit = async () => {
  try {
    ...
    uni.redirectTo({ url: `/pagesOrder/detail/detail?id=${result.id}` })
  } catch (err) {
...
<template>
  <scroll-view scroll-y class="viewport">
    <!-- 收货地址 -->
    <navigator v-if="selectAddressData" class="shipment" hover-class="none" url="/pagesMember/address/address?from=order">
      <view class="user"> {{ selectAddressData?.receiver }} {{ selectAddressData?.contact }} </view>
      <view class="address"> {{ selectAddressData?.fullLocation }} {{ selectAddressData?.address }} </view>
    ...
    <!-- 商品信息 -->
    <view class="goods">
      <navigator v-for="item in orderPre?.goods" :key="item.skuId" :url="`/pages/goods/goods?id=${item.id}`" class="item" hover-class="none">
